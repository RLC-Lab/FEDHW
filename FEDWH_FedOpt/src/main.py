...
from server import inference, lstm_inference, FedadamOptimizer, cosine_annealing_lr, get_server_energy, get_gpu_power
from energy_simulator import cal_energy
from latency_simulator import cal_latency


if __name__ == "__main__":
    print("FedOptEF Start!")
    np.random.seed(17)
    ...
    # set up the FedOptAdam optimizer
    opt_kwargs = dict(lr=args.serverlr, betas=(0.9, 0.999), v0=0.00001, tau=0.001)
    server_optimizer = FedadamOptimizer(global_model.parameters(), **opt_kwargs)
    server_optimizer.add_param_group(dict(params=list(global_model.buffers())))
    ...
    lr = args.lr
    scheduler = cosine_annealing_lr(init_lr=lr, max_lr=lr, min_lr=1e-6, epochs=args.rounds)
    ...
    while accuracy < args.accuracy:
        ...
        for client in chosen_client:
            ...
            if args.model == 'cnn' or args.model == 'resnet18' or args.model == 'audio':
                local_layers_iterator, loss, duration = local_client.train(args, local_model, train_dataset,
                                                                           user_train_groups[client],
                                                                           epochs, batch_size, iteration, lr)
            elif args.model == 'lstm':
                local_layers_iterator, loss, duration = local_client.lstm_train(args, local_model, train_dataset,
                                                                                user_train_groups[client],
                                                                                epochs, batch_size, iteration, lr)
            local_layers_iterators.append(local_layers_iterator)
        ...
        if args.energyOpt is not None:
            ...
            ag_time1 = time.time()
            for local_layers_iterator in local_layers_iterators:
                server_optimizer.accumulate(chosen, local_layers_iterator)
            ...
            ag_time2 = time.time()
            ag_duration = ag_time2 - ag_time1
            local_latency = cal_latency(args, iteration, epochs, batch_size, fraction, global_weights, ag_duration,
                                        latency_sum)
        else:
            for local_layers_iterator in local_layers_iterators:
                server_optimizer.accumulate(chosen, local_layers_iterator)
        ...
        with open(path, 'a+') as f:
            f.write("Global Training Iteration: {} \t Test Accuracy: {:.2f}% \t Train Loss: {:.5f}\n".format(
                iteration, 100 * accuracy, np.mean(np.array(local_losses))))
            f.write("Epoch: {} \t Batch size: {} \t Fraction: {} \t Learning rate: {:.5f}\n".format(epochs, batch_size,
                                                                                                fraction, lr))
        ...
        if accuracy < args.accuracy:
            if args.energyOpt == 'SA':
                ...
                epochs, batch_size, fraction = clm.Latency_sim_annealing(args, epochs, batch_size, fraction,
                    former_accuracy, accuracy, global_model.state_dict(), ag_duration, local_latency, latency_sum)
    ...
    with open(path, 'a+') as f:
        ...
        f.write(f"Server optimizer learning rate: {args.serverlr}\n")
